...
export default function LiquidBalanceSection() {
  ...
  const handleDelete = async (sourceId: string) => {
    if (confirm('¿Está seguro de eliminar esta fuente de balance líquido?')) {
      await dispatch(deleteBalanceSource(sourceId));
    }
  };
    ...
    <div className='bg-white rounded-lg shadow-lg p-6 mb-6'>
      <div className='flex justify-between items-center mb-4'>
        <div>
          ...
          <p className='text-sm text-zinc-600 mt-1'>
            Dinero que puedes utilizar para gastos
          </p>
        </div>
        <Button
      ...
      {balanceSources.length === 0 ? (
        <p className='text-zinc-600 text-center py-8'>
          No hay fuentes de balance líquido configuradas
        </p>
      ) : (
        ...
        <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
          <div className='bg-white rounded-lg p-6 w-full max-w-md mx-4'>
            <h4 className='text-xl font-bold mb-4 text-primary-dark'>
              {editingId ? 'Editar Fuente' : 'Nueva Fuente de Balance Líquido'}
            </h4>
